...
resource "aws_route53_record" "mws-a-record" {
  count = var.solution_enable["management"] == "true" ? var.instancecount_integration : 0
...
resource "aws_instance" "mws" {
  count = var.solution_enable["management"] == "true" ? var.instancecount_mws : 0
  ...
  tags = merge(
    ...
    local.linux_tags,
    {
      "Name" = "${local.name_prefix}-mws-${data.aws_subnet.COMMON_APPS[count.index].availability_zone}"
