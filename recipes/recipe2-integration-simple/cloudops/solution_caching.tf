...
resource "aws_route53_record" "terracotta-a-record" {
    count = "${lookup(var.solution_enable, "caching") == "true" ? var.instancecount_terracotta : 0}"

    zone_id = "${data.aws_route53_zone.main-internal.zone_id}"
    name = "${local.name_prefix}-terracotta${count.index+1}.${data.aws_route53_zone.main-internal.name}"
    type = "A"
    ttl  = 300
    records = [
...
resource "aws_instance" "terracotta" {
  ...
  tags = "${merge(
    ...
    local.linux_tags,
    map(
      "Name", "${local.name_prefix}-terracotta-${data.aws_subnet.COMMON_APPS.*.availability_zone[count.index]}",
