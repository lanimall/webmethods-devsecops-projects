...
resource "aws_instance" "apigateway" {
  ...
  tags = merge(
    ...
    {
      "Name" = "${local.name_prefix_long}-apigateway${count.index + 1}-${data.aws_subnet.COMMON_WEB[count.index].availability_zone}"
...
resource "aws_lb_target_group" "apigateway-ui" {
  ...
  name                 = "${local.name_prefix_unique_short}-apigateway-ui"
  port                 = 9072
  protocol             = "HTTP"
  ...
  tags = merge(
    local.common_tags,
    {
      "Name" = "${local.name_prefix_long}-apigateway-ui"
    },
  )
}
...
resource "aws_lb_target_group" "apigateway-runtime" {
  ...
  name                 = "${local.name_prefix_unique_short}-apigateway-runtime"
  port                 = 5555
  protocol             = "HTTP"
  vpc_id               = data.aws_vpc.main.id
  slow_start           = 100
  ...
  tags = merge(
    local.common_tags,
    {
      "Name" = "${local.name_prefix_long}-apigateway-runtime"
    },
  )
}
