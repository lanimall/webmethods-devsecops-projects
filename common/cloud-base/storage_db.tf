...
resource "aws_security_group" "maindb" {
  ...
  ingress {
    ...
    cidr_blocks = [
      ...
      "${aws_subnet.COMMON_APPS.cidr_block}"
    ]
  }
...
resource "aws_db_instance" "maindb" {
  ...
  kms_key_id             = "${aws_kms_key.maindb.arn}"
  ...
  db_subnet_group_name   = "${aws_db_subnet_group.maindb.name}"
  parameter_group_name   = "${aws_db_option_group.maindb.name}"
  option_group_name      = "${aws_db_parameter_group.maindb.name}"
